...
def conv1d(inputs,
    ...
    with tf.variable_scope(scope):
        ...
        if padding.lower() == "causal":
            ...
            pad_len = rate-1-(shape[1]+rate-1) % rate  # padding size
            inputs = tf.pad(inputs, [[0, 0], [0, pad_len], [0, 0]])
            inputs=tf.reshape(inputs,[hp.batch_size,-1,hp.Q])
            #print(inputs)
            padding = "valid"
            causal=True
        ...
        params = {"inputs": inputs, "filters": filters, "kernel_size": size,
                  ...
                  "use_bias": use_bias, "reuse": reuse, "kernel_initializer": tf.truncated_normal_initializer()}
        ...
        if causal == True:
            out_width=shape[1]-(size-1)*rate
            tensor = tf.slice(tensor,[0,0,0],[-1,out_width,-1])
            tensor = tf.reshape(tensor,[hp.batch_size,out_width,hp.Q*2])
        if onebyone == True:
            tensor = tf.reshape(tensor,[hp.batch_size,shape[1],hp.Q])
    return tensor
